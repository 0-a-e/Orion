...
public interface GridCells {
    ...
    public static final class Adaptive implements GridCells {
        ...
        private Adaptive(float f) {
            ...
            if (Dp.m6613compareTo0680j_4(f, Dp.m6614constructorimpl((float) 0)) <= 0) {
                throw new IllegalArgumentException(("Provided min size " + Dp.m6625toStringimpl(f) + " should be larger than zero.").toString());
            }
        }

        public List<Integer> calculateCrossAxisCellSizes(Density density, int i, int i2) {
            return LazyGridDslKt.calculateCellsCrossAxisSizeImpl(i, Math.max((i + i2) / (density.m6545roundToPx0680j_4(this.minSize) + i2), 1), i2);
        }
    ...
    public static final class FixedSize implements GridCells {
        ...
        private FixedSize(float f) {
            this.size = f;
            if (Dp.m6613compareTo0680j_4(f, Dp.m6614constructorimpl((float) 0)) <= 0) {
                throw new IllegalArgumentException(("Provided size " + Dp.m6625toStringimpl(f) + " should be larger than zero.").toString());
            }
        }

        public List<Integer> calculateCrossAxisCellSizes(Density density, int i, int i2) {
            int r3 = density.m6545roundToPx0680j_4(this.size);
            int i3 = r3 + i2;
            int i4 = i2 + i;
            if (i3 < i4) {
                int i5 = i4 / i3;
