...
final class IntrinsicHeightNode extends IntrinsicSizeModifier {
    ...
    public long m619calculateContentConstraintsl58MMJ0(MeasureScope measureScope, Measurable measurable, long j) {
        int i;
        if (this.height == IntrinsicSize.Min) {
            i = measurable.minIntrinsicHeight(Constraints.m6526getMaxWidthimpl(j));
        } else {
            i = measurable.maxIntrinsicHeight(Constraints.m6526getMaxWidthimpl(j));
        }
        if (i < 0) {
            i = 0;
        }
        return Constraints.Companion.m6535fixedHeightOenEA2s(i);
    }
