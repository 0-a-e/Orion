...
public final class MultiParagraphKt {
    ...
    public static final void m5918findParagraphsByRangeSbBc2M(List<ParagraphInfo> list, long j, Function1<? super ParagraphInfo, Unit> function1) {
        int findParagraphByIndex = findParagraphByIndex(list, TextRange.m6040getMinimpl(j));
        int size = list.size();
        while (findParagraphByIndex < size) {
            ParagraphInfo paragraphInfo = list.get(findParagraphByIndex);
            if (paragraphInfo.getStartIndex() < TextRange.m6039getMaximpl(j)) {
