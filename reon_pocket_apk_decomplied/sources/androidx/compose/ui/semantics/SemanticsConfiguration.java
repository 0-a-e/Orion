...
public final class SemanticsConfiguration implements SemanticsPropertyReceiver, Iterable<Map.Entry<? extends SemanticsPropertyKey<?>, ? extends Object>>, KMappedMarker {
    ...
    public final boolean containsImportantForAccessibility$ui_release() {
        Iterable<SemanticsPropertyKey> keySet = this.props.keySet();
        if ((keySet instanceof Collection) && ((Collection) keySet).isEmpty()) {
            return false;
        }
        for (SemanticsPropertyKey isImportantForAccessibility$ui_release : keySet) {
            if (isImportantForAccessibility$ui_release.isImportantForAccessibility$ui_release()) {
                return true;
            }
        }
        return false;
