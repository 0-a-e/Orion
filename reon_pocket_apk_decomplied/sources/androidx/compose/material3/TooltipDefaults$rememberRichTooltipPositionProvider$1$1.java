...
public final class TooltipDefaults$rememberRichTooltipPositionProvider$1$1 implements PopupPositionProvider {
    ...
    public long m2603calculatePositionllwVHH4(IntRect intRect, long j, LayoutDirection layoutDirection, long j2) {
        int right = intRect.getRight();
        if (IntSize.m6790getWidthimpl(j2) + right > IntSize.m6790getWidthimpl(j) && (right = intRect.getLeft() - IntSize.m6790getWidthimpl(j2)) < 0) {
            right = intRect.getLeft() + ((intRect.getWidth() - IntSize.m6790getWidthimpl(j2)) / 2);
        }
