...
public class KeyProvider18 implements KeyProvider {
    ...
    public synchronized Key generateKey(String str) throws KeyNotGeneratedException {
        ...
        try {
            ...
            throw new KeyNotGeneratedException("Error in generating the RSA Encryption key for the rsaKeyAlias: " + str + " in AndroidKeyStore", e);
        } catch (Exception e2) {
            throw new KeyNotGeneratedException("Error in generating the AES key and RSA encrypting the AES key using the rsaKeyAlias: " + str + " in AndroidKeyStore", e2);
        } catch (Throwable th) {
            throw th;
        }
        return generateKey;
    ...
    public synchronized void deleteKey(String str) {
        ...
        try {
            ...
            Log log = logger;
            log.error("Error in deleting the RSA Key identified by the keyAlias: " + str + " from AndroidKeyStore", e2);
        }
    }
}
