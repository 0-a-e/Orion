...
public class BucketConfigurationXmlFactory {
    ...
    public byte[] convertToXmlByteArray(BucketNotificationConfiguration bucketNotificationConfiguration) {
        ...
        for (Map.Entry next : bucketNotificationConfiguration.getConfigurations().entrySet()) {
            String str = (String) next.getKey();
            NotificationConfiguration notificationConfiguration = (NotificationConfiguration) next.getValue();
            if (notificationConfiguration instanceof BucketNotificationConfiguration.TopicConfiguration) {
                ...
                xmlWriter.start(JsonDocumentFields.POLICY_ID).value(str).end();
                xmlWriter.start("Topic").value(((BucketNotificationConfiguration.TopicConfiguration) notificationConfiguration).getTopicARN()).end();
    ...
    private void addEventsAndFilterCriteria(XmlWriter xmlWriter, NotificationConfiguration notificationConfiguration) {
        ...
        if (filter != null) {
            ...
            if (filter.getS3KeyFilter() != null) {
                ...
                for (FilterRule next : filter.getS3KeyFilter().getFilterRules()) {
                    xmlWriter.start("FilterRule");
                    xmlWriter.start("Name").value(next.getName()).end();
    ...
    public byte[] convertToXmlByteArray(BucketReplicationConfiguration bucketReplicationConfiguration) {
        ...
        for (Map.Entry next : rules.entrySet()) {
            ReplicationRule replicationRule = (ReplicationRule) next.getValue();
            xmlWriter.start("Rule");
            xmlWriter.start("ID").value((String) next.getKey()).end();
            xmlWriter.start(TextFieldImplKt.PrefixId).value(replicationRule.getPrefix()).end();
    ...
    private void writeRule(XmlWriter xmlWriter, TagSet tagSet) {
        ...
        for (String next : tagSet.getAllTags().keySet()) {
            xmlWriter.start("Tag");
            xmlWriter.start("Key").value(next).end();
            xmlWriter.start("Value").value(tagSet.getTag(next)).end();
            xmlWriter.end();
        }
        xmlWriter.end();
    }
